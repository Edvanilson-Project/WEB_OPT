#!/usr/bin/env python3
"""Test greedy algorithm only (no hybrid)"""
...
print(f"Testing GREEDY algorithm (simplest)")
...
try:
    result = service.run(
        ...
        algorithm=AlgorithmType.GREEDY,  # Use greedy, not hybrid!
        cct_params={"max_shift_minutes": 480, "strict_hard_validation": False},
        vsp_params={"min_layover_minutes": 10},
        time_budget_s=30,  # Limit to 30 seconds
    )
    elapsed = time.time() - start
    print(f"\n✓ Greedy optimization completed in {elapsed:.2f}s")
    print(f"  Vehicles: {len(result.vsp.blocks)}")
    print(f"  Crew: {len(result.csp.duties)}")

except Exception as e:
    elapsed = time.time() - start
    print(f"\n✗ Greedy optimization failed after {elapsed:.2f}s: {type(e).__name__}: {e}")
    import traceback
    traceback.print_exc()

print("\nNow testing GREEDY with strict_hard_validation=True...")
start = time.time()
try:
    result = service.run(
        trips=list(trips),
    ...
    elapsed = time.time() - start
    print(f"\n✓ Greedy (strict=True) completed in {elapsed:.2f}s")
    print(f"  Vehicles: {len(result.vsp.blocks)}")
    print(f"  Crew: {len(result.csp.duties)}")

except Exception as e:
    elapsed = time.time() - start
    print(f"\n✗ Greedy (strict=True) failed after {elapsed:.2f}s: {type(e).__name__}: {e}")
