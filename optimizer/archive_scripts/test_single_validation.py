...
def _trip(trip_id, start, duration, line=1, origin=1, dest=2, depot=1):
    return Trip(
        id=trip_id,
        line_id=line,
        start_time=start,
...
# Simulate check_daily_shift_limit from qa_professional_validation_2026.py
...
try:
    result = service.run(
        ...
        time_budget_s=30.0,  # Same as in test_greedy_only.py
    )
    ...
    print(f"✓ Validation check completed in {elapsed:.2f}s")
    print(f"  max_spread={max(spreads) if spreads else 0} duties={len(spreads)}")
    print(f"  Passed: {ok}")

except Exception as e:
    elapsed = time.time() - start
    print(f"✗ Validation check failed after {elapsed:.2f}s: {type(e).__name__}: {e}")
    import traceback
    traceback.print_exc()
