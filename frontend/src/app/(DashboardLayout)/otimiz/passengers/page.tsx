...
import {
  IconPlus, IconEdit, IconTrash, IconRefresh, IconUsers, IconDeviceFloppy,
...
function PassengersInner() {
  ...
  const [configs, setConfigs] = useState<PassengerConfig[]>([]);
  const [lines, setLines] = useState<Line[]>([]);
  const [loading, setLoading] = useState(true);

  const [selected, setSelected] = useState<PassengerConfig | null>(null);
  const [bands, setBands] = useState<PassengerBand[]>([]);
  const [bandsLoading, setBandsLoading] = useState(false);
  const [bandsDirty, setBandsDirty] = useState(false);
  const [savingBands, setSavingBands] = useState(false);

  const [dialogOpen, setDialogOpen] = useState(false);
  const [editTarget, setEditTarget] = useState<PassengerConfig | null>(null);
  const [form, setForm] = useState({ lineId: '', description: '', bandIntervalMinutes: '30', startHourMinutes: '240', endHourMinutes: '1440' });
  const [saving, setSaving] = useState(false);

  const [deleteTarget, setDeleteTarget] = useState<PassengerConfig | null>(null);
  ...
  const loadBands = useCallback(async (cfg: PassengerConfig) => {
    setSelected(cfg);
    setBandsLoading(true);
    setBandsDirty(false);
    try {
      const data = await passengerConfigsApi.getBands(cfg.id);
  ...
  const openEdit = (cfg: PassengerConfig) => {
    setEditTarget(cfg);
    setForm({
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (editTarget) {
        await passengerConfigsApi.update(editTarget.id, {
        ...
        const created = await passengerConfigsApi.create(payload);
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await passengerConfigsApi.delete(deleteTarget.id);
  ...
  const updateBand = (idx: number, field: keyof PassengerBand, value: string) => {
    setBands((prev) =>
  ...
  const saveBands = async () => {
    ...
    setSavingBands(true);
    try {
      await passengerConfigsApi.saveBands(
        selected.id,
        bands.map((b) => ({
          startMinutes: b.startMinutes,
          endMinutes: b.endMinutes,
          passengersOutbound: b.passengersOutbound ?? 0,
          passengersReturn: b.passengersReturn ?? 0,
        })),
      );
  ...
  return (
    <PageContainer title="Passageiros — OTIMIZ" description="Configuração de passageiros por faixa">
      <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
        <Box>
          <Typography variant="h4" fontWeight={700} lineHeight={1}>Passageiros por Faixa</Typography>
          <Typography variant="body2" color="text.secondary" mt={0.5}>
            Configure a quantidade de passageiros (ida/volta) por faixa horária
          </Typography>
        </Box>
                ...
                <Table size="small" stickyHeader>
                  <TableHead>
                    <TableRow>
                      ...
                      <TableCell align="center" sx={{ fontWeight: 600, minWidth: 130 }}>Passageiros Ida</TableCell>
                      <TableCell align="center" sx={{ fontWeight: 600, minWidth: 130 }}>Passageiros Volta</TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    {bands.map((band, idx) => (
                      <TableRow key={idx} hover>
                        ...
                        <TableCell align="center">
                          <TextField
                            ...
                            size="small"
                            sx={{ width: 80 }}
                            value={numVal(band.passengersOutbound ?? 0)}
                            onChange={(e) => updateBand(idx, 'passengersOutbound', e.target.value)}
                            inputProps={{ min: 0 }}
                          />
                        </TableCell>
                        <TableCell align="center">
                          <TextField
                            ...
                            size="small"
                            sx={{ width: 80 }}
                            value={numVal(band.passengersReturn ?? 0)}
                            onChange={(e) => updateBand(idx, 'passengersReturn', e.target.value)}
                            inputProps={{ min: 0 }}
                          />
                        </TableCell>
                      </TableRow>
...
export default function PassengersPage() {
  return <NotifyProvider><PassengersInner /></NotifyProvider>;
}
